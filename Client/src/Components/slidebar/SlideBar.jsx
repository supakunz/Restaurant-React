...
const SlideBar = ({ toggle, setToggle }) => {
  ...
  return (
    <>
      <section>
        <div className={`fixed bg-white h-screen w-[26rem] top-[70px] ${toggle ? 'right-0' : '-right-[30rem]'} z-50 p-5 transition-all duration-300 shadow-2xl`}>
          ...
          <div className='border-solid border-t-[1px] border-black '>
            ...
            <div className='flex justify-between items-center gap-4 text-white'>
              <Link to={'/cart'} className='flex-1'>
                <button onClick={() => { window.scrollTo(0, 0); setToggle(false) }} className='bg-yellow w-full h-[2.3rem] rounded-md hover:bg-yellowHover'>Cart</button>
              </Link>
              <Link to={'/chechout'} className='flex-1'>
                <button onClick={() => { window.scrollTo(0, 0); setToggle(false) }} className='bg-yellow w-full h-[2.3rem] rounded-md hover:bg-yellowHover'>Check out</button>
              </Link>
            </div>
          </div>
        </div>
